...
async function loadSolvedExams(userId) {
  try {
    ...
    if (result.success) {
      ...
      result.solvedExams.forEach(async (exam) => {
        const examRef = await getDoc(doc(db, "exams", exam.examId));
        if (!examRef.exists()) {
          return;
        }
        ...
        examElement.innerHTML = `
          <div class="bg-white rounded-lg shadow-md p-4 mb-4 font-semibold text-lg">
            ...
            <div class="mb-1">
              <span class="font-medium">رقم الاختبار:</span> ${exam.examName}
            </div>
            <div class="mb-1 ${scoreColor}">
              النتيجة: ${exam.score} / ${exam.questions.length}
            </div>
            <div class="mb-1">
              <span class="font-medium">تم الحل في:</span> ${exam.solvedAt}
            </div>
            <div>
...
loadSolvedExams(getLocStore("uid"));
