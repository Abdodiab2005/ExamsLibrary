...
app.get("/get-solved-exams/:userId", async (req, res) => {
  ...
  try {
    ...
    const resultsCollectionRef = admin.firestore().collection("results");
    ...
    for (const subcollection of subcollectionsSnapshot) {
      ...
      examsSnapshot.forEach((examDoc) => {
        allExamData.push({
          examId: examDoc.id, // Get the examId (random ID)
          ...examDoc.data(), // Get the exam data
          parentSubcollection: subcollection.id, // Add the parent subcollection ID
        });
      });
    }
